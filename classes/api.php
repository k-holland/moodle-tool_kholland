...
/**
 * Class tool_kholland_api for various api methods
 *
 * @package    tool_kholland
 * @copyright  2018 Karen Holland
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class tool_kholland_api {
    ...
    public static function retrieve(int $id, int $courseid = 0, int $strictness = MUST_EXIST) {
        ...
        return $DB->get_record('tool_kholland', $params, '*', $strictness);
    }
    ...
    public static function update(stdClass $data) {
        ...
        if (isset($data->description_editor)) {
            $data = file_postupdate_standard_editor($data, 'description',
                self::editor_options(), $PAGE->context, 'tool_kholland', 'entry', $data->id);
        }
        ...
        $DB->update_record('tool_kholland', $updatedata);
    }
    ...
    public static function insert(stdClass $data) : int {
        ...
        $entryid = $DB->insert_record('tool_kholland', $insertdata);

        // Id known, so description data next.
        if (isset($data->description_editor)) {
            $context = context_course::instance($data->courseid);
            $data = file_postupdate_standard_editor($data, 'description',
                self::editor_options(), $context, 'tool_kholland', 'entry', $entryid);
    ...
    public static function delete(int $id) {
        ...
        $DB->delete_records('tool_kholland', ['id' => $id]);
    }
